...
class Mods
{
	...
	inline public static function directoriesWithFile(path:String, fileToFind:String, mods:Bool = true)
	{
		...
		if(mods)
		{
			...
			{
				var currentFolder:String = Paths.mods(Mods.currentModDirectory + '/' + fileToFind);

				if(FileSystem.exists(currentFolder) && !foldersToCheck.contains(currentFolder))
					foldersToCheck.push(currentFolder);
			}
		}
		#end
	...
	public static function getModchartPath(song:String, ?folder:String = null):String
	{
		#if MODS_ALLOWED
		...
		var modFolder:String = Paths.mods(folder + '/modcharts/');
		var globalFolder:String = Paths.mods('modcharts/');

		if(!FileSystem.exists(modFolder))
			FileSystem.createDirectory(modFolder);

		if(!FileSystem.exists(globalFolder))
			FileSystem.createDirectory(globalFolder);

		var modchartPath:String = modFolder + songPath + '.json';

		if(FileSystem.exists(modchartPath))
			return modchartPath;

		var globalPath:String = globalFolder + songPath + '.json';
